...
const DebtTable: React.FC<DebtTable> = props => {
  const { debts, openDelete, openEdit, openStatus, openDetail, isLoading } =
    props
    ...
    <div className='rounded container bg-white py-2 px-4'>
      <div className='my-4'>
                ...
                {debts.map((debtor: Debt, index: number) => (
                  <tr key={debtor._id} className='hover:bg-gray-100'>
                    <td className='border-y border-slate-100 p-2'>
                      ...
                      <div className='text-xs text-slate-500'>{debtor._id}</div>
                    </td>
                    <td className='border-y border-slate-100 p-2 text-sm'>
                      {currencyFormat(debtor.amount)}
                    </td>
                    <td className='border-y border-slate-100 p-2 text-sm w-32'>
                      {debtor.method}
                    </td>
                    <td className='border-y border-slate-100 p-2 text-sm w-32'>
                      {dateFormat(debtor.date_created, 'YYYY-MM-DD')}
                    </td>
                    <td
                      className={`border-y border-slate-100 p-2 text-sm w-32 ${ifDateIsDue(debtor.due_date) ? 'bg-red-500 text-white font-semibold' : ''}`}
                    >
        ...
        {debts.length === 0 && !isLoading && (
          <div className='border-b border-x border-slate-100 p-2 text-center text-sm text-slate-400 font-semibold'>
            No data
          </div>
        )}
      </div>
...
const statusBtnTxt = (data: string) =>
  data.toLowerCase() === 'paid' ? 'Unpaid' : 'Paid'
